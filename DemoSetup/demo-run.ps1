...
if ($StartServers) {
    ...
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd `"$RepoPathackend`"; python -m uvicorn app.main:app --reload --port 8001"
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd `"$RepoPathrontend`"; `$env:PORT=3001; `$env:NEXT_PUBLIC_API_BASE=`"http://localhost:8001`"; npm run dev"
